from puzzle import RandomPuzzle as RP
class Node:
    ...
    def goal(self):
        ...
        for i in range(0,self.n):
            for j in range(1,self.n+1):
                temp = [str(k) for k in range(1+i*self.n,self.n+1+i*self.n)]
            goal.append(temp)
        goal[self.n-1][self.n-1] = "_"
        return goal
    def accept(self):
        """ Accepts the puzzle from the user """
        puz = []
        for i in range(0,self.n):
            temp = self.puzzle[0+self.n*i:self.n + self.n*i]
            puz.append(temp)
        return puz
...
randomMatrix = RP.RandomMatrix(4)
srcMatrix = randomMatrix.GetPuzzle()
testMatrix = list(flatten(srcMatrix))
testMatrix = [str(i) for i in testMatrix]
realMatrix = []
for text in testMatrix:
    if (text == '0'):
        realMatrix.append('_')
    else:
        realMatrix.append(text)
# testMatrix = [w.replace('0', '_', 1) for w in testMatrix]
print(realMatrix)

puz = Puzzle(4, realMatrix)
# puz = Puzzle(3,["7","6","1","3","2","8","4","5","_"])
